...
def RedNeuronal():
    ...
    host = '82.180.172.103'  # Cambia esto al host de tu base de datos
    user = 'u292477693_root'    # Cambia esto al usuario de tu base de datos
    password = 'LJwG?Ub7.'  # Cambia esto a la contraseña de tu base de datos
    db_name = 'u292477693_rugby'  # Cambia esto al nombre de tu base de datos
    ...
    try:
     ...
     with connection.cursor() as cursor:
         ...
         features = np.array(features)  # Convertir a arreglo de NumPy
         targets = np.array(targets)   
         
         scaler = StandardScaler()
         features = scaler.fit_transform(features)  # Normalización de características
    ...
    capaOculta  = tf.keras.layers.Dense(units=7) # Capa oculta con 3 neuronas para los calculos
    ...
    modelo.fit(features, targets, epochs=500, verbose=False)
